...
-- The inserts are skipped on an empty table so seeding still owns fresh databases.
...
INSERT OR IGNORE INTO `feature_flags` (`id`, `key`, `description`, `owner`, `environment`, `enabled`, `updated_at`)
...
WHERE EXISTS (SELECT 1 FROM `feature_flags`);--> statement-breakpoint
INSERT OR IGNORE INTO `feature_flags` (`id`, `key`, `description`, `owner`, `environment`, `enabled`, `updated_at`)
